...
class MyListData : AppCompatActivity(), RecyclerViewAdapter.dataListener {
    ...
    override fun onDeleteData(data: data_mahasiswa?, position: Int) {
        /* Kode ini akan dipanggil ketika method onDeleteData dipanggil dari adapter pada RecyclerView * melalui interface. kemudian akan menghapus data berdasarkan primary key dari data tersebut * Jika berhasil, maka akan memunculkan Toast */
        val getUserID: String = auth?.getCurrentUser()?.getUid().toString()
        val getReference = database.getReference()
